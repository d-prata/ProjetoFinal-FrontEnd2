...
function adicionarNovoRecado() {
    ...
    if (existe) {
        alert("O código já foi cadastrado para outro recado!");
        inputCodigo.value = '';
        inputCodigo.focus();
...
function preencherTabela(recado: Recado) {
    ...
    botaoApagar.setAttribute('class', 'btn btn-danger me-2');
    ...
    colunaCodigo.innerHTML = `<h3 class="fs-5 text-center">${recado.codigo}</h3>`;
    colunaDescricao.innerHTML = `<p class="fs-5">${recado.descricao}</p>`;
...
function apagarRecado(codigo: number) {
    ...
    let confirma: boolean = confirm(`Você tem certeza que deseja excluir o recado ${codigo}?`);
